...
// Hard
// Method: Double loop. We fix the first point and find the points with the
// same slope. unordered_map of pairs, each pair represents a slope. We use GCD
// to avoid using doubles. overload of operators
 ...
 public:
  ...
  int maxPoints(vector<vector<int>>& points) {
	...
	// if current point is a duplicate
        if(points[i][0] == points[j][0] && points[i][1] == points[j][1]) ++duplicate;
	  ...
	  if(line_slope.find(slope) == line_slope.end()) line_slope[slope] = 1;
          else line_slope[slope] += 1;
        }
      }

...
public:
    //Method 1: DP
    //When k >= n / 2; it is converted to Best Time to But and Sell Stock I, time complexity is only O(N)
    //Faster
    int maxProfit(int k, vector<int>& prices) {
        ...
        for (int i = 1; i <= k; ++i) {
            ...
            res = max(res, sells.back());
        }
        return res;
    }

    //Method 2: use a priority to store all the profits, use a a stack to store all the peak-valley pair
    //When dealing with 2 adjacent pairs, if the valley of the former one is lower than the latter one's, we need to transform the presentation of the profits of these two(or more, that's why we use stack)
    int maxProfit(int k, vector<int>& prices) {

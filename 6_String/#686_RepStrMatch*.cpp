...
public:
    int repeatedStringMatch(string A, string B) {
        ...
        //entry in this table means when this match fails, which position in B should we go to. -1 means this rep fails, we also need to change the pos in A.
        vector<int> helper(sB);
        helper[0] = -1;
        //pos means the next char in our search (the suffix of the substring so far), cnd means the next candidate char to match (which is the end of the prefix of B)
        int pos = 1, cnd = 0;
        while (pos < sB) {
            if (B[pos] == B[cnd]) {
        ...
        while (1) {
            if (A[pA % sA] == B[pB]) {
                ...
                //when this match fails, we go back to last candidate, but still keep pos in A
                pB = helper[pB];
